...
```{r setup}
knitr::opts_knit$set(root.dir = normalizePath("/Users/helending/Documents/BCB330/code/normal_tissues/lung"))
```
# Library
```{r}
library(knitr)
...
URL <- "https://www.ebi.ac.uk/arrayexpress/files/E-GEOD-48414"
SDRF.file <- "E-GEOD-48414.sdrf.txt"
Data.file <- "E-GEOD-48414.raw.1.zip"
...
norm_ind <- which(SDRF$`Comment [Sample_source_name]` == "Normal")
length(norm_ind)
```
# Read data
```{r}
...
# save final
```{r}
unique_all <- process_human_miR(defined, norm_ind=c(1, 1, 1))
save_known_and_novel(unique_all_table=unique_all, tissue_name="lung")
```