...
---
```{r setup}
knitr::opts_knit$set(root.dir = normalizePath("/Users/helending/Documents/BCB330/code/normal_tissues/prostate")) 
```
# Library
```{r}
library(knitr)
...
```{r, results="hide"}
URL <- "https://www.ebi.ac.uk/arrayexpress/files/E-GEOD-21036"
SDRF.file <- "E-GEOD-21036.sdrf.txt"
Data.file <- "E-GEOD-21036.raw.1.zip"
...
all_sample <- SDRF[,c("Array Data File","Characteristics[disease status]")]
norm_ind <- which(SDRF$`Characteristics[disease status]` == "normal adjacent benign prostate")
```
# Read data
```{r}
read_data <- read_data_by_limma(norm_ind)
read_data <- data.frame(read_data)
read_data <- read_data[,-c(1, 2, 3, 4, 5, 6, 8)]
```
...
save_known_and_novel(unique_all_table=unique_all, tissue_name="prostate")
```