...
# helper files
```{r}
source("agilent_workflow_helper.R")
```
...
```{r}
# threshold: 
greatest_0.01 <- define_expression_threshold(y_neg_ctrl=cleaned, norm_ind=length(norm_ind))
...
# save final
```{r}
unique_all <- process_human_miR(defined, tissue_name="heart")
save_known_and_novel(unique_all_table=unique_all, tissue_name="heart")
```
